...
{
public:
  LidarObjectDetectionNode()
  : Node("lidar_object_detection_node", rclcpp::NodeOptions().enable_logger_service(true)),
    next_object_id_(0)
  {
    // --- Parameters ---
...
private:
  ...
  bool is_object_in_tracking_zone(
    const Point2D & center, double detection_angle_deg, double buffer_distance) const
  {
    double detection_angle_rad = detection_angle_deg * M_PI / 180.0;
    double min_angle_rad = -detection_angle_rad / 2.0;
    double max_angle_rad = detection_angle_rad / 2.0;
    ...
    if (object_distance == 0.0) return true;  // At origin
    ...
    return object_angle >= (min_angle_rad - angle_buffer) &&
           object_angle <= (max_angle_rad + angle_buffer);
  }

  OrientedBoundingBox compute_oriented_bounding_box(
    const std::vector<geometry_msgs::msg::Point32> & points)
  {
    ...
    for (size_t i = 0; i < points.size(); ++i) {
      for (size_t j = i + 1; j < points.size(); ++j) {
        double dist_sq =
          std::pow(points[i].x - points[j].x, 2) + std::pow(points[i].y - points[j].y, 2);
  ...
  void scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
  {
    ...
    publish_tracking_zone_marker(
      msg->header, detection_angle_deg_, buffer_distance, msg->range_max);
    ...
    auto candidates =
      filter_clusters(clusters, min_object_points_, min_object_width_, max_object_width_);
  ...
  std::vector<Point2D> scan_to_points(
    const sensor_msgs::msg::LaserScan::SharedPtr & msg, double angle_deg)
  {
  ...
  std::vector<std::vector<Point2D>> group_points(
    const std::vector<Point2D> & points, double max_dist_between_points)
  {
  ...
  std::vector<ObjectCandidate> filter_clusters(
    const std::vector<std::vector<Point2D>> & clusters, int min_points, double min_width,
    double max_width)
  {
    std::vector<ObjectCandidate> candidates;
    for (const auto & cluster : clusters) {
      if (cluster.size() < static_cast<size_t>(min_points)) continue;
      double object_width =
        std::hypot(cluster.back().x - cluster.front().x, cluster.back().y - cluster.front().y);
      if (object_width < min_width || object_width > max_width) continue;
      ...
      for (size_t i = 0; i < candidates.size(); ++i) {
        if (!candidate_matched[i]) {
          double dist = std::hypot(
            track.center_.x - candidates[i].center.x, track.center_.y - candidates[i].center.y);
          if (dist < min_dist) {
            min_dist = dist;
  ...
  void publish_objects(const std_msgs::msg::Header & header, double detection_angle_deg)
  {
    ...
    std::sort(sorted_tracks.begin(), sorted_tracks.end(), [](const auto & a, const auto & b) {
  ...
  void publish_markers(const std_msgs::msg::Header & header, double detection_angle_deg)
  {
    ...
    for (const auto & track : tracked_objects_) {
      ...
      if (track.bounding_box_.empty()) continue;
  ...
  /**
     ...
     */
  void publish_fov_marker(
    const std_msgs::msg::Header & header, double detection_angle_deg, float range_max)
  {
  ...
  void publish_tracking_zone_marker(
    const std_msgs::msg::Header & header, double detection_angle_deg, double buffer_distance,
    float range_max)
  {
  ...
  void update_config()
  {
    ...
    if (min_object_width_ <= 0.0 || min_object_width_ >= max_object_width_) {
      RCLCPP_ERROR(
        this->get_logger(), "min_object_width_m must be positive and less than max_object_width_m");
      return;
    }
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Configuration updated - Detection angle: %.1f°, Object width: %.2f-%.2fm",
...
const std::string LidarObjectDetectionNode::PARAM_TRACKING_DIST_THRESHOLD =
